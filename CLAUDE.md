...
Plus a **temporary scaffold**: **Browser level runtime preview** ([preview/](preview/), Phaser via CDN lazy-load). Lets the designer "walk around" a freshly-edited `.tmj` in the browser. Plays sprite walking animation when player moves (heuristic state_key lookup). Movement keys also sprite-driven (4-dir sprites only allow WASD). **State animations driven via PC remote console** ([console.html](console.html)) — designer opens it on a desktop browser to drive the dog (DPad + state strip + playback) on the phone for video recording. Mobile shows clean canvas only — no DPad, no info text, no prompt. Console talks to phone via `_https_server.py`'s `/api/control` endpoint (in-memory relay, ThreadingHTTPServer + 50ms short-poll). **Will be removed** once cute_pet engineer ships `lib/demo/level_preview.dart` (preview/ + console.html + the /api/control endpoint all go together). Do NOT let `preview/` accrete business logic — it's a bridge, not a long-term home.
...
- 2026-05-08: PC remote console for video recording. `console.html` (DPad + state strip + playback) drives the phone via `/api/control` short-poll relay (`_https_server.py` gains a 25-line `Handler` + `ThreadingHTTPServer`). preview/ gains **forced state** machinery (Tab clears, Digit1-9 pick, Space toggles play/pause, [/] step) — fully overrides the walk/idle heuristic when active. Mobile DPad + info + prompt all hidden in level mode (clean canvas for video). Player body footprint reverted from over-shrunk 1/3×1/5 to standard 1/2×1/4 (per-shape collision now handles tight gaps). All gated to "录像专用" use case; preview/ + console.html + /api/control are scrapped together when cute_pet ships its demo entry.
- 2026-05-17: port handoff to nginx. cute pixel console (`console.ewow.cn`) shares the same box and needs :443 for SSL termination + host-based vhost. asset-lab moves to `:8001` behind nginx reverse-proxy; external `https://1.14.190.95/` URL unchanged for the designer. Only `_https_server.py` PORT + a few doc comments changed in repo; nginx install / cert / vhost config done by cute pixel side. See [handoff doc](https://github.com/471402921/consle/blob/main/handoff/asset-lab.md).
...
- **Mobile touch UI = none in level mode.** The mobile DPad / zoom DOM lives in `index.html` (`#level-touch`) but `preview/main.js` no longer wires it — was replaced by PC remote console (see next section). `preview/main.js` hides `#info` / `#prompt` / `#level-touch` on mobile so video recording captures a clean canvas.
...
- **When user says "cute_pet engineer wrote demo entry":** propose `git rm -r preview/ console.html` + strip the `/api/control` endpoint from `_https_server.py` (revert to 12-line SimpleHTTPServer) + revert `index.html` to single sprite-preview entry + mark plan §14.7 as "removed" + flip `docs/cute_pet_integration.md` accordingly. Do NOT keep `preview/` "just in case."

### PC remote console rules (`console.html` + `/api/control`)

The PC remote console exists *only* to make recording demo videos easier. Designer puts the phone in front of a camera (clean canvas, full-bleed map), opens `https://1.14.190.95/console.html` on a desktop browser, and drives the dog from PC keyboard / DPad while the phone shows the result.

- **Same lifecycle as `preview/`.** When `preview/` is deleted, `console.html` and the `/api/control` endpoint go too. Don't keep them.
- **Single global session.** `_state` is one in-memory dict, no rooms / multi-tenancy. Acceptable because there's one designer recording at a time. If two PCs hit it simultaneously, they'd race — that's by design (don't add session IDs).
- **Protocol = polling, not push.** Phone short-polls `/api/control` every 50ms via `LevelPreviewMode._startPolling`. Hold-state (movement keys) is overwritten verbatim each tick. Triggers (state / anim / zoom) use a monotonic `seq` counter — phone consumes only events with `seq > lastSeq`. **Do NOT switch to SSE / WebSocket** — the simplicity is the point; latency (~100ms) is fine for video.
- **Forced state in preview overrides walk/idle heuristic.** When console picks Digit1-9, `forcedStateKey` is set; `_pickAnimKey` returns that state's anim regardless of movement. Tab clears (back to heuristic). Movement still works (position changes) but anim stays locked. This matches sprite_preview behavior; designer is in explicit control during recording.
- **`_https_server.py` grew from 12 → ~45 lines.** Two reasons documented in its top comment: TLS bypass + `/api/control` relay. ThreadingHTTPServer is a one-line stdlib drop-in (no new deps). Don't grow further without a clear need.
...
- `preview/main.js` — level-preview mode (Phaser via CDN, **temporary scaffold**, see preview/README.md). Self-contained, doesn't reuse `core/`/`loaders/` deliberately. Plays walking anim heuristically (state_key contains 'walk'); idle anim if state_key contains 'idle'/'stand'/'breath'. **PC remote console** (console.html) drives forced state / movement / zoom via `/api/control` short-poll relay (~50ms cycle).
- `console.html` — **temporary scaffold**, single-file PC remote control panel (DPad + state strip + playback). Imports `keymap.js` for keyboard semantics, fetches sprite metadata for state list, POSTs to `/api/control`. Keep it lean; ditch with `preview/` when cute_pet ships its demo entry.
- `_https_server.py` — TLS-wrapped HTTP server (`/etc/systemd/system/asset-lab-https.service` runs it). Serves static files; also exposes `/api/control` GET/POST for the PC remote console. ThreadingHTTPServer so 50ms phone polls don't queue behind static-file requests. Stdlib only.
