...
### 改地图 / 关卡(纯 Tiled, asset-lab 不参与转换)

```
1. (可选) pixellab 生成基础 tile / 家具 / UI 单 PNG → 拷进 assets/ 对应位置
2. Tiled 里手画 / 拼地图 / 配 collision / 摆 NPC
3. Tiled 导出 .tmx → 放 assets/ (具体路径设计师定)
4. git commit + push
5. cute_pet 工程师 git pull → flame_tiled 自动加载新关卡
```
...
├── tilesets/                        # 设计师在 Tiled 里建的 .tsx + tile PNG
...
- **不转换地图** — 设计师在 Tiled 里直接画 + 导出 .tmx,asset-lab 不参与
- **不重造 Tiled** — 关卡编辑全部交给 Tiled
- **不替 cute_pet 写 Flutter / Dart 代码** — asset-lab 只出 spec
- **不引擎化** — 物理 / 碰撞 / 状态机 / 触发器都是 cute_pet 的事
