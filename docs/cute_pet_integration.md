...
> - [x] sprite 状态切换 + 动画播放在 asset-lab 实装 — **2026-05-05 yellow_Shiba 触发实装**, schema 见本 doc §1.1。**待设计师把 state_key 从 prompt-derived 长串改成 semantic 名 (`idle` / `walking` / ...)**, 才闭环 readiness
>
> **当前 known-unstable 区**:
...
| `assets/sprites/{name}/metadata.json` | pixellab 角色元数据 (含 8 方向 rotations + 多段 animations) | pixellab 导出 | cute_pet **可直接读** (sprite 动画播放, 见 §1.1) |
| `assets/sprites/{name}/rotations/{dir}.png` | 角色 8 方向 PNG (静帧) | pixellab 导出 | (通过 .tsx 间接用 + animation fallback) |
...
### 1.1 sprite metadata.json animations schema (2026-05-05 第一只 sprite 真实测)

```jsonc
{
  "frames": {
    "rotations": { "south": "rotations/south.png", /* 8 方向 */ },
    "animations": {
      "<state_key>": {
        "<direction>": [
...
- **`state_key`**: opaque ID, 由设计师在 pixellab 命名 (semantic: `idle` / `walking` / `sleeping` / `lying` / `crouch`)。**asset-lab 不维护 alias 表**, cute_pet 也按 key 名直接消费。
- **`direction`**: pixellab 8 方向 literal 字符串 (`south` / `south-east` / `east` / `north-east` / `north` / `north-west` / `west` / `south-west`)。**不翻译为 N/E/S/W**。
- **direction 覆盖可不全**: 同一动画下不同方向**可独立缺失**。yellow_Shiba 实测: walking 有 south/east/west, idle/sleep/lie/crouch 都只有 south。
...
| 谁负责 | 内容 |
|---|---|
| **pixellab** | 生成基础 PNG 元素(地形 tile / 家具 / UI 单图)+ 角色 sprite 8 方向 + 状态/动画帧(2026-05-05 实测到位, 见 §1.1) |
| **asset-lab** | git 资源管理 + sprite `.tsx` 转换 + sprite web 预览 (含 8 方向 + 状态切换 + 动画播放) |
