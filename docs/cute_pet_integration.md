...
> - [ ] 第一只完整 sprite 跑通: pixellab → `assets/sprites/{name}/` → `python3 tools/pixellab_to_tiled.py --sprites` 出 `.tsx` → 设计师在 Tiled 里把 NPC 摆进 .tmx
> - [ ] 至少一张 .tmx + 引用资源跑通 cute_pet 的 `flame_tiled` 加载(含 collision)
...
> - .tmx 内 layer 命名 / object 类型 / tile property 约定(§3)— 等设计师做完真关卡
> - flame_tiled 加载示例(§3)— 大方向稳定,但具体 layer name 要等 §3 约定填实
> - cute_pet 改动清单粒度(§6)
>
> **当前 known-stable 区**:
> - asset-lab 不产 .tmx — 设计师在 Tiled 里直接画并导出
...
> - cute_pet 用 `flame_tiled` 加载 .tmx(标准 Flame 生态用法)
> - 资源更新流程的高层闭环(§5)
...
- 设计师在 **Tiled** 里画的关卡 `.tmx` + 引用的 PNG
- pixellab 角色 sprite + asset-lab 自动生成的 sprite `.tsx`(供 Tiled 引用)
- 音频文件

cute_pet 用 `flame_tiled` 一行加载 .tmx,collision / sprite 摆位等都按 Tiled 标准方式自动跟随。**asset-lab 不替 cute_pet 写 Dart**,本文是 cute_pet 工程师的 spec 参考。

```
asset-lab 仓 (asset 源)               cute_pet 仓 (Flame 游戏)
─────────────────────────             ───────────────────────────
设计师在 Tiled 编辑 .tmx       ────►  flame_tiled.TiledComponent.load()
...
> ⚠️ 子目录最终布局**由设计师定**(她在 Tiled 工程里习惯什么布局,asset-lab 跟随)。下表是当前预期,gate 1/4 落地后会校正。

| asset-lab 路径 | 内容 | 来源 | cute_pet 用途 |
|---|---|---|---|
| `assets/maps/{name}.tmx` | Tiled 关卡 | 设计师在 Tiled 导出 | `TiledComponent.load('maps/{name}.tmx', ...)` |
| `assets/maps/` 下 / `assets/tilesets/` 下 PNG | .tmx 引用的 tile / 背景图 | 设计师 (pixellab 出基础 PNG 元素 → 设计师拼) | flame_tiled 自动加载 |
| `assets/tilesets/*.tsx` | Tiled tileset 定义 | 设计师在 Tiled 里建 | flame_tiled 自动 follow .tmx 引用 |
...
| `assets/sprites/{name}/{name}.tsx` | Tiled image collection,8 个 tile,带 `direction` property | **asset-lab converter 生成** | 在 Tiled 里把 NPC 摆进关卡时引用,cute_pet 加载 .tmx 时自动认识 |
...
| **设计师 + Tiled** | 拼地图 / 配 collision / 摆 NPC / 关卡编辑 / 导出 .tmx |
| **cute_pet** | 用 `flame_tiled` 加载 .tmx + 业务逻辑 + 状态机 + 音频播放 + 玩家输入 |

asset-lab **不**做的:
- 不产 .tmx(设计师产)
- 不写 Dart 代码(cute_pet 工程师按本文写)
- 不约束 .tmx 内的 layer 命名 / tile property(设计师 + cute_pet 协议,asset-lab 转述)
...
> ⚠️ 具体 layer 名要等 §3 约定填实后才能写实例。下面给出**通用骨架**,cute_pet 工程师按设计师实际产出的 .tmx 修改 layer 名。
...
设计师在 Tiled / pixellab 改东西
   ↓
设计师 git push asset-lab (改 .tmx / sprite / audio 直接 commit)
   ↓
cute_pet 工程师 git pull asset-lab (或 submodule update / rsync)
   ↓
cute_pet hot reload — flame_tiled 重读 .tmx, 无需改一行 Dart 代码
```
